  ...
  <div style="margin-top: 16px ">
    <a-row>
      <a-col :span="16">
        ...
        ></a-button>
        <a-button @click="fetchData()" :loading="loading" shape="circle" icon="reload"/>
      </a-col>
      <a-col :span="8" v-if="!$route.params || !$route.params.id">
        <a-input-search size="default" placeholder="Search" @search="onSearch">
      ...
      <a-spin :spinning="action.loading">
        <a-form :form="form" @submit="handleSubmit" layout="vertical">
          ...
          >
            ...
            <span v-else-if="field.type==='uuid' || field.name==='account'">
              <a-select
                ...
                :placeholder="field.description"
              >
                <a-select-option v-for="(opt, index) in field.opts" :key="index">{{ opt.name }}</a-select-option>
              </a-select>
            </span>
          ...
          <a-form-item>
            ...
            >
              <a-button style="marginRight: 8px" @click="closeAction">Cancel</a-button>
              <a-button :loading="action.loading" type="primary" html-type="submit">Submit</a-button>
            </div>
          </a-form-item>
        </a-form>
      </a-spin>
    ...
    <div v-else>
      ...
      >
        <p slot="expandedRowRender" slot-scope="record" style="margin-right: 5px">
          {{record.id}}
          <a-row>
            <a-col :span="16">
              <a-button
